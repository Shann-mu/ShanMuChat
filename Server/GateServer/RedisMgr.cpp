...
bool RedisMgr::Set(const std::string& key, const std::string& value)
{
    ...
    //如果返回NULL说明执行失败
    if (NULL == reply) {
        std::cout << "Ececut command [ SET " << key << " " << value << " ] failed ! " << std::endl;
        freeReplyObject(reply);
        return false;
    }
    ...
    //执行成功 释放redisCommand执行返回的redisReply所占用的内存
    freeReplyObject(reply);
    std::cout << "Execut command [ SET " << key << "  " << value << " ] success ! " << std::endl;
    return true;
}
...
bool RedisMgr::LPush(const std::string& key, const std::string& value)
{
    ...
    if (NULL == reply) {
        std::cout << "Excut command [ PLUSH " << key << " " << value << " ] failure ! " << std::endl;
        freeReplyObject(reply);
        return false;
    }
...
bool RedisMgr::RPush(const std::string& key, std::string& value)
{
    auto connect = _con_pool->getConnection();
    if (connect == nullptr) {
        return false;
